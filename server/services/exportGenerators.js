...
function generateFilename(summary, extension) {
  if (!summary || typeof summary !== 'string') {
    return `mind-mesh-summary.${extension}`
  }
  
  // Take first 50 characters, remove special chars, convert to kebab-case
  let title = summary
    .substring(0, 50)
    .trim()
    .toLowerCase()
...
export async function generateOne(format, summaryObj) {
  ...
  switch (format) {
    case 'txt':
      ...
      return bufferResult(generateFilename(summary, 'txt'), 'text/plain', Buffer.from(txtContent, 'utf8'))
    case 'pdf':
      return await generatePdf(summary, highlights, metadata)
    case 'docx':
      return await generateDocx(summary, highlights, metadata)
    case 'pptx':
      return await generatePptx(summary, highlights, metadata)
    case 'png':
    case 'image':
      return await generateImage(summary, metadata)
    default:
...
async function generatePdf(summary, highlights, metadata = {}) {
  ...
  doc.end()
  await done
  return bufferResult(generateFilename(summary, 'pdf'), 'application/pdf', Buffer.concat(chunks))
}

async function generateDocx(summary, highlights, metadata = {}) {
  const paragraphs = []
  ...
  return bufferResult(generateFilename(summary, 'docx'), 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', buffer)
}

async function generatePptx(summary, highlights, metadata = {}) {
  ...
  return bufferResult(generateFilename(summary, 'pptx'), 'application/vnd.openxmlformats-officedocument.presentationml.presentation', Buffer.from(buffer))
}

async function generateImage(summary, metadata = {}) {
  ...
  await browser.close()
  return bufferResult(generateFilename(summary, 'png'), 'image/png', buffer)
}
